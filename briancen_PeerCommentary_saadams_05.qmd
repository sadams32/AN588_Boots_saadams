...
yongliang's comment: I appreciate that you load both curl and tidyverse. In my code I used read.csv() directly with a URL which is a bit simpler but using curl can also be a solid choice if you need additional control.
...
yongliang's comment: Your display of the first few rows with head(d) is great for ensuring the data loaded as expected.
...
yongliang's comment: I really like how you used dplyr’s filter() to clean the data. This is similar to my approach with subsetting. Creating new columns for the log-transformed values is very clear and is similar to my approch. I directly use log inside the lm().
...
yongliang's comment: Your linear model runs smoothly and the summary() output provides a clear overview of the results. I also printed the beta coefficients in my code using cat() for clarity.
...
yongliang's comment: Your creation of a dedicated dataframe for storing bootstrap coefficients is clear. I did similar thing where I used a matrix to store these coefficients
...
Yongliang's comment: your loop is really easy to follow!
...
Yongliang's comment: I reached similar conclusions in my homework, and this comparison is a really clear method to validate the results.
...
Yongliang's comment: Nice work converting the model formula from a character string with as.formula(). I use a similar approach. Your function is well-structured and clear.
...
Yongliang's final comment: I think your code is super solid overall and it is really clear and well documented. I have found several places where i did things similar to yours and I found that reassuring that we are on the right track.
